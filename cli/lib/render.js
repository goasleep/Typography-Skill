...
// Heading inline element overrides — inherit heading color, prevent theme style leak
    ...
    highlight(str, lang) {
      ...
      if (lang && hljs.getLanguage(lang)) {
        try {
          ...
          codeContent = MarkdownIt.prototype.utils.escapeHtml(str);
        }
      } else {
        codeContent = MarkdownIt.prototype.utils.escapeHtml(str);
      }
...
function simplifyForClipboard(html, styleKey) {
  ...
  // markdown-it wraps highlight output in <pre><code>, so structure is:
  // <pre><code class="language-xxx"><div style="border-radius: 8px;">...</div></code></pre>
  // We need to replace the entire outer <pre> with a clean <pre> containing macOS dots + <code>
  const macosDots = '<div style="display: flex; align-items: center; gap: 6px; padding: 10px 12px; background: #2a2c33; border-bottom: 1px solid #1e1f24;"><span style="width: 12px; height: 12px; border-radius: 50%; background: #ff5f56;"></span><span style="width: 12px; height: 12px; border-radius: 50%; background: #ffbd2e;"></span><span style="width: 12px; height: 12px; border-radius: 50%; background: #27c93f;"></span></div>';
  ...
  codeBlocks.forEach(block => {
    ...
    if (codeElement) {
      ...
      // Walk up to the outermost <pre> that markdown-it created and replace it
 ...
 * @param {string} styleKey - Style theme key (e.g. 'wechat-default')
 * @param {object} options - { clipboard: boolean } — if true, apply clipboard-compatible simplifications
 * @returns {string} WeChat-compatible HTML
 */
...
module.exports = { render, getAvailableStyles, STYLES };
