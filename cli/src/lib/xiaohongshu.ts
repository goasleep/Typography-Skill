...
import type { ContentBlock, ArticleInfo, GenerateImagesOptions, GenerateImagesResult, PagesHTMLOptions } from './types';
...
function createSimplifiedContent(markdown: string): ContentBlock[] {
...
function calculateArticleInfo(markdown: string): ArticleInfo {
...
function getBackgroundColor(styleKey: string): string {
...
function infoPanelHTML(articleInfo: ArticleInfo): string {
  return `
...
function pageNumberHTML(current: number, total: number): string {
  return `<div style="position:absolute;bottom:30px;right:40px;font-size:14px;color:#999;font-weight:500;">${current}/${total}</div>`;
}

function buildMeasurementHTML(blocks: ContentBlock[], bgColor: string): string {
...
function buildPagesHTML(pages: ContentBlock[][], options: PagesHTMLOptions): string {
...
function buildScreenshotHTML(pagesHTML: string, bgColor: string): string {
  return `<!DOCTYPE html>
...
async function splitIntoPages(blocks: ContentBlock[], bgColor: string): Promise<ContentBlock[][]> {
  ...
  const heights = await page.evaluate(() => {
    ...
    return Array.from(blocks).map(b => (b as HTMLElement).offsetHeight);
  });
