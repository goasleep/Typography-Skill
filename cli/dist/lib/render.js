    ...
    return new markdown_it_1.default({
        ...
        highlight(str, lang) {
            ...
            if (lang && highlight_js_1.default.getLanguage(lang)) {
                try {
                    codeContent = highlight_js_1.default.highlight(str, { language: lang }).value;
                }
                catch {
...
function applyInlineStyles(html, styleKey) {
    const styleConfig = styles_1.STYLES[styleKey];
    if (!styleConfig) {
        throw new Error(`Unknown style: "${styleKey}". Available: ${Object.keys(styles_1.STYLES).join(', ')}`);
    }
        ...
        for (let j = 0; j < columns; j++) {
            ...
            if (index < imgWrappers.length) {
                ...
                if (img) {
                    ...
                    wrapper.setAttribute('style', `width: 100% !important; height: ${containerHeight} !important; text-align: center !important; background-color: #f5f5f5 !important; border-radius: 4px !important; padding: 10px !important; box-sizing: border-box !important; overflow: hidden !important; display: table !important;`);
                    ...
                    newImg.setAttribute('style', `max-width: calc(100% - 20px) !important; max-height: ${imgMaxHeight} !important; width: auto !important; height: auto !important; display: inline-block !important; margin: 0 auto !important; border-radius: 4px !important; object-fit: contain !important;`);
...
function simplifyForClipboard(html, styleKey) {
    const styleConfig = styles_1.STYLES[styleKey];
    const dom = new jsdom_1.JSDOM(html);
    const doc = dom.window.document;
    // Grid → Table
    ...
    if (containerBg && containerBg !== '#fff' && containerBg !== '#ffffff') {
        ...
        section.setAttribute('style', `background-color: ${containerBg}; padding: ${padding}; max-width: ${maxWidth}; margin: 0 auto; box-sizing: border-box; word-wrap: break-word;`);
...
function render(markdownContent, styleKey = 'wechat-default', options = {}) {
...
function getAvailableStyles() {
    return Object.entries(styles_1.STYLES).map(([key, config]) => ({
        key,
        name: config.name,
    }));
}
